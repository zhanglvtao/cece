...
## Aiden Chat Completions → Responses API 转换的 input_text Bug（2026-06-18 已规避）

### 问题现象

cece 使用 gpt-5.2-pro/gpt-5.4-pro/codex 等 OpenAI 模型时，通过 Aiden 代理发送 Chat Completions 请求，偶发 400 Bad Request：
...
cece 发送 Chat Completions (`/v1/chat/completions`) 请求，Aiden 代理服务端对 GPT 系列模型需要转为 OpenAI Responses API。转换使用 langchain 的 `g()` 函数，该函数**不区分 message role**，对所有 text content 统一标记为 `input_text`：

```javascript
// Aiden g() 函数的 bug
for(let n of e.contentBlocks)
  if("text"===n.type) c([{type:"input_text",text:n.text}]); // 应该根据 role 区分 output_text/input_text
```

而 OpenAI Responses API 要求 assistant 消息的 text content 类型必须是 `output_text`，不接受 `input_text`。

Aiden CLI 自己不会触发此 bug，因为它直接走 Responses API（`useResponsesApi: true`），用 `_()` 函数转换（正确区分 `output_text`/`input_text`）。

Aiden 中 `_()` 和 `g()` 的路由判断：
```javascript
_ = ({messages, zdrEnabled, model}) => messages.flatMap(e => {
  if (e.response_metadata?.output_version === "v1") return g(e); // 走有 bug 的 g()
  // ... 否则走 _() 的正确逻辑
})
```
...
cece 实现 OpenAI Responses API 协议，直接发 `/v1/responses` 请求（`forceSanitizedReplay` 模式，每次传全量历史），绕过 Aiden 服务端的 `g()` 转换。

cece 自己控制消息序列化，正确区分：
- user/developer 消息的 text → `input_text`
- assistant 消息的 text → `output_text`
- tool_result → `function_call_output`（含 `call_id`）
- tool_use → `function_call`（含 `call_id`）

所有 `gpt-*` 和 `o1/o3/o4-*` 模型自动走 Responses API。

### 关键源码

- `internal/aiden/responses_serialize.go` — 请求序列化（对应 Aiden `_()` 函数逻辑）
- `internal/aiden/responses_stream.go` — SSE 流式解析
- `internal/aiden/client.go` — `Stream()` 分支：`useResponsesAPI` → `/v1/responses`
- `cmd/cece/main.go` — `shouldUseResponsesAPI()` 自动判断
